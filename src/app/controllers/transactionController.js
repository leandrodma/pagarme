const {CREDIT, DEBIT} = require('../../config/config')
	...
	async create(req, res, next) {
		...
		try {
			...
			if( newTransaction.payment_method === DEBIT.CARD ){
				await clientTransaction.update({ available_funds: clientTransaction.available_funds + newPayable.value })
			}else{
				await clientTransaction.update({ waiting_funds: clientTransaction.waiting_funds + newPayable.value })
			}
